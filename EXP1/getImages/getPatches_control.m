...
save_folder = '/bwlab/Users/SeoheeHan/NSDData/rothzn/nsd/Orientation/experiment/implicitStimuli';
...
for imgIdx = 1:550
    ...
    imgnumColumn = repmat(imgNum(imgIdx), numValidPatches, 1); % Repeat imgNum for each patch

    % Store each valid patch as a separate row
    resultsTable = table(imgnumColumn, imgIdx_names, validPatches(:,1), validPatches(:,2), ...
        validMeans_photo, rad2deg(validMeans_photo), ...
        validMeans_vecLD, rad2deg(validMeans_vecLD), ...
        validDiff, rad2deg(validDiff), ...
        'VariableNames', {'imgnum', 'imgIdx_name', 'xShift', 'yShift', ...
        'validMeans_photo', 'validMeans_photo_deg', ...
        'validMeans_vecLD', 'validMeans_vecLD_deg', ...
        'validDiff', 'validDiff_deg'});
...
save(fullfile(original_folder, 'results_tableTotal.mat'), 'allResultsTable');
