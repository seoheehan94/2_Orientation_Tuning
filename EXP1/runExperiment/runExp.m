...
try % error handling start (error -> automatically close all screens)
    ...
    saveFileName = ['results/', expName, '_results_' participantID '.mat'];
    if exist(saveFileName, 'file')
        load(saveFileName, 'data', 'lastTrialNumber', 'lastPhase', 'practiceAngles');
        startTrial = lastTrialNumber + 1;
        startPhase = lastPhase;
        outputFile = fopen(['results/', expName, '_results_' participantID '.csv'], 'a');
    else
        startTrial = 1;
        startPhase = 'practice'; % Start with practice phase if no previous state
        data = struct('participantID', {},'trialPhase', {}, 'trialNumber', {}, 'imageName', {}, 'imageDuration', {}, 'rotationAngle', {}, 'startOrientation', {},'responseOrientation', {}, 'barResponseTime', {}, 'confidenceLevel', {});
        outputFile = fopen(['results/', expName, '_results_' participantID '.csv'], 'w');
        fprintf(outputFile, 'participantID,trial_phase,trial,imageName,imageDuration,rotationAngle,startOrientation,response_orientation,bar_response_time,confidenceLevel\n'); % Header

    end

    % Load participant condition file
    conditionFile = fullfile('participant_conditions', sprintf('sub_%02d_conditions.mat', str2double(participantID)));
    if exist(conditionFile, 'file')
        load(conditionFile, 'participantTrials'); % Load the conditions file
    else
    ...
    notice2 = ['In this experiment, you will view a series of images.\n' ...
        ...
        'There will be 4 blocks in total.\n\n' ...
    ...
    yCenter = rect(4)/2; % Y-coordinate for screen center
    ...
    % choose 10 random rotation angle
    ...
    if strcmp(startPhase, 'practice')        
        ...
        for trial = startTrial:nTrials_prac
            ...
            trialData = struct('participantID', participantID, 'trialPhase', 'practice', 'trialNumber', trial, ...
                'imageName', imgName, 'imageDuration', imageDuration, ...
            ...
            data = [data; trialData];
            save(saveFileName, 'data','lastTrialNumber','lastPhase', 'practiceAngles'); % Save data to .mat file

            % Save csv
            fprintf(outputFile, '%s,practice,%d,%s,%.4f,%d,%.4f,%.4f,%.4f,%d\n', ...
                participantID, trial, imgName, imageDuration, rotationAngle, startOrientation, responseOrientation, barResponseTime, confidenceLevel);
        end
    ...
    for trial = startTrial:nTrials
       ...
       imageDuration = barResponseTime + unlimitPresentation;
        ...
        % Save csv
        fprintf(outputFile, '%s,main,%d,%s,%.4f,%d,%.4f,%.4f,%.4f,%d\n', ...
            participantID, trial, imgName, imageDuration, rotationAngle, startOrientation,responseOrientation, barResponseTime,confidenceLevel);
